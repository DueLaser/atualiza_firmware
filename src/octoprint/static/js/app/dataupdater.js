function DataUpdater(allViewModels) {
    ...
    self._onclose = function(e) {
        ...
        if (self._autoReconnectTrial >= self._autoReconnectDialogIndex) {
            ...
            showOfflineOverlay(
                gettext("Servidor offline"),
                gettext("O servidor parece estar offline. Tentarei reconectar <strong>em breve</strong>, no entanto pode tentar manualmente a qualquer momento."),
                self.reconnect
            );
        }
    ...
    self._onreconnectfailed = function() {
        ...
        $("#offline_overlay_title").text(gettext("Servidor offline"));
            ...
            switch (prop) {
                ...
                case "event": {
                    ...
                    if (type == "MovieRendering") {
                        ...
                        new PNotify({title: gettext("Conversão terminada"), text: _.sprintf(gettext("Converteu %(stl)s para %(gcode)s, em %(time).2f segundos"), payload), type: "success"});
                        ...
                        html = _.sprintf(gettext("Não foi possivel converter %(stl)s para %(gcode)s: %(reason)s"), payload);
