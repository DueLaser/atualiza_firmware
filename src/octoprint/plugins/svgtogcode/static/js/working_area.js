		...
		self.placeSVG = function(file) {
			...
			callback = function (f) {
				...
				if(clipPathEl.length != 0){
					console.warn("Aviso: Elementos clipPath removidos do desenho!");
				...
				f.node.childNodes.forEach(function(entry) {
					if(entry.nodeType == 8) { // Nodetype 8 = comment
						if(entry.textContent.indexOf('Illustrator') > -1) {
							new PNotify({title: gettext("Illustrator SVG detectado"), text: "Illustrator SVG detectado! Para preservar as dimensões mude o DPI para 72 em settings - conversão SVG e adicione novamente o arquivo!", type: "info", hide: false});
						}
					}
				});
...
//			});
		...
		self.svgTransformUpdate = function(svg){
			...
			$('#'+label_id+' .translation').text('x: '+ tx.toFixed(1) + ' , '+ 'y: ' + ty.toFixed(1));
			$('#'+label_id+' .horizontal').text(horizontal.toFixed() + 'mm');
			$('#'+label_id+' .vertical').text(vertical.toFixed()+ 'mm');
			$('#'+label_id+' .rotation').text(rot.toFixed(1)+'º');
		};
		
		self.mudaAltura = function(file){
			...
			console.log(tx.toFixed(1));
			console.log(ty.toFixed(1));



			fitMatrix.translate(-tx.toFixed(1)*2/globalScale+pos_x*2/globalScale,(ty.toFixed(1)-pos_y)*2/globalScale);
 			fitMatrix.add(svg.transform().localMatrix);
 			svg.transform(fitMatrix);

			fitMatrix = new Snap.Matrix();	
			bbox = svg.getBBox();
			ty = self.workingAreaHeightMM() - self.px2mm(bbox.y2 * globalScale);
			tx = self.px2mm(bbox.x * globalScale);
			fitMatrix.translate(-tx.toFixed(1)*2/globalScale+pos_x*2/globalScale,(ty.toFixed(1)-pos_y)*2/globalScale);
			fitMatrix.add(svg.transform().localMatrix);
			svg.transform(fitMatrix);
		...
		self.svg_contains_clipPath_warning = function(){
            var error = "<p>" + gettext("O arquivo contém elementos clipPath.<br/>Estes elementos ainda não são suportados e foram removidos.") + "</p>";
            //error += pnotifyAdditionalInfo("<pre>" + data.jqXHR.responseText + "</pre>");
            new PNotify({
                title: "clipPath removidos",
                text: error,
                type: "warn",
                hide: false
            });
		...
		self.svg_contains_text_warning = function(svg){
            var error = "<p>" + gettext("O Arquivo contem texto.<br/> Se quiser marcar/cortar somente a parte exterior,<br/>por favor converte para caminho.") + "</p>";
            //error += pnotifyAdditionalInfo("<pre>" + data.jqXHR.responseText + "</pre>");
            new PNotify({
                title: "Elementos de Texto encontrados",
                text: error,
                type: "warn",
                hide: false
            });
		...
		self.svg_misfitting_warning = function(svg, misfitting){
            ...
            var error = "<p>" + gettext("O design não cabia na área de trabalho e foi redimensionado.")
					+ outside + oversized + gettext("<br/>Por favor cheque o resultado.") + "</p>";
            new PNotify({
                title: "Design moved",
                text: error,
                type: "warn",
