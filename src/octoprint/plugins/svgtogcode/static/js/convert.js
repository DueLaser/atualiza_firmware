$(function(){
	...
	function VectorConversionViewModel(params) {
		...
		self.maxSpeed = ko.observable(100);
		self.minSpeed = ko.observable(1);
		...
		self.imgIntensityWhite = ko.observable(1);;
		self.imgIntensityBlack = ko.observable(100);;
		self.imgFeedrateWhite = ko.observable(100);; 
		self.imgFeedrateBlack = ko.observable(1);;
		...
		self.convert = function() {
			if(self.gcodeFilesToAppend.length === 1 && self.svg === undefined){
				...
				self.workingArea.getCompositionSVG(self.fill_areas(), function(composition){
					...
					if (self.laserSpeed()<1) self.laserSpeed(1);					
					if (self.laserSpeed()>100) self.laserSpeed(100);				
				
					if (self.laserIntensity()<1) self.laserIntensity(1);					
					if (self.laserIntensity()>100) self.laserIntensity(100);
					
					if (self.imgIntensityWhite()<1) self.imgIntensityWhite(1);					
					if (self.imgIntensityWhite()>100) self.imgIntensityWhite(100);
					
					if (self.imgIntensityBlack()<1) self.imgIntensityBlack(1);					
					if (self.imgIntensityBlack()>100) self.imgIntensityBlack(100);
					if (self.imgIntensityWhite()>self.imgIntensityBlack()) self.imgIntensityWhite(self.imgIntensityBlack());					
				
					if (self.imgFeedrateBlack()<1) self.imgFeedrateBlack(1);					
					if (self.imgFeedrateBlack()>100) self.imgFeedrateBlack(100);
					
					
					
					var potencia=parseInt((self.laserIntensity()*890)/100)+100;
					var potencia_branco=parseInt((self.imgIntensityWhite()*890)/100)+100;
					var potencia_preto=parseInt((self.imgIntensityBlack()*890)/100)+100;
					var velocidade_imagem=parseInt((self.imgFeedrateBlack()*5000)/100)+100;
					var velocidade=parseInt((self.laserSpeed()*5000)/100)+100;
					
					console.log(potencia);
					console.log(potencia_branco);
					console.log(potencia_preto);
					console.log(velocidade_imagem);
					...
					var data = {
						...
						"profile.intensity": potencia,
						"profile.fill_areas": self.fill_areas(),
						"profile.pierce_time": self.pierceTime(),
						"profile.intensity_black" : potencia_preto,
						"profile.intensity_white" : potencia_branco,
						"profile.feedrate_black" : velocidade_imagem,
						"profile.feedrate_white" : velocidade_imagem,
...
//			time: 30.612739086151123
		...
		self._configureFeedrateSlider = function() {
			...
			var speedSubscription = self.laserSpeed.subscribe(function(fromSettings){
				...
				self.feedrateSlider.slider('setValue', realVal);
		...
		self._calcRealSpeed = function(sliderVal){
			return sliderVal;
/* 			Math.round(self.minSpeed() + sliderVal/100 * (self.maxSpeed() - self.minSpeed()));
 */		};
