...
class LaserCutterProfileManager(object):
	...
	def _load_from_path(self, path):
		...
		if not profile:
			self._logger.warn("Invalido: %s" % path)
			raise InvalidProfileError()
		return profile
	...
	def _save_to_path(self, path, profile, allow_overwrite=False):
		...
		if os.path.exists(path) and not allow_overwrite:
			raise SaveError("Modelo %s já existe" % profile["id"])
		...
		with open(path, "wb") as f:
			try:
				...
				raise SaveError("Não foi possível salvar %s: %s" % (profile["id"], e.message))
	...
	def _load_default(self, defaultModel = None):
		...
		if(defaultModel is not None and defaultModel == "_mrbeam_senior"):
			default['volume']['width'] = 300
			default['volume']['depth'] = 300
			default['model'] = "Pro"
			default['id'] = "_mrbeam_senior"
		
		profile = self._ensure_valid_profile(default)
		if not profile:
			self._logger.warn("Invalido")
			raise InvalidProfileError()
		return profile
	...
	def _sanitize(self, name):
		...
		if "/" in name or "\\" in name:
			raise ValueError("Não pode conter / ou \\")
